...
chutSetup:SetScript("OnEvent", function(self, event, ...)
    if event == "PLAYER_LOGIN" then
        CHUT = CHUT or {}
        ChatUtils:SetVersion(133457, "0.3.93")
        ...
        ChatUtils:CreateMinimapButton({
            ...
            ["var"] = mmbtn,
            ["dbtab"] = CHUT,
            ["vTT"] = {{"|T133457:16:16:0:0|t ChatUtils", "v" .. ChatUtils:GetVersion()}, {ChatUtils:Trans("LID_LEFTCLICK"), ChatUtils:Trans("LID_OPENSETTINGS")}, {ChatUtils:Trans("LID_RIGHTCLICK"), ChatUtils:Trans("LID_HIDEMINIMAPBUTTON")}},
...
function ChatUtils:InitSettings()
    ...
    cu_settings = ChatUtils:CreateWindow({
        ...
        ["sw"] = 520,
        ["sh"] = 520,
        ["title"] = format("|T133457:16:16:0:0|t ChatUtils v%s", ChatUtils:GetVersion())
    })
    ...
    ChatUtils:AppendCheckbox("SHOWMINIMAPBUTTON", ChatUtils:GetWoWBuild() ~= "RETAIL", function()
    ...
    if UnitGroupRolesAssigned then ChatUtils:AppendCheckbox("SHOWROLEICON", true) end
